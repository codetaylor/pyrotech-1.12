...
public class ScrapingPeltRecipe
    extends ShapelessOreRecipe {

  public ScrapingPeltRecipe(ResourceLocation group, NonNullList<Ingredient> input, @Nonnull ItemStack result) {
  ...
  public NonNullList<ItemStack> getRemainingItems(InventoryCrafting inv) {
    ...
    for (int i = 0; i < result.size(); i++) {
      ...
      if (OreDictHelper.contains("toolHuntersKnife", itemStack)) {
        ...
        if (itemStack.attemptDamageItem(ModuleHuntingConfig.HIDE_SCRAPING_TOOL_DAMAGE, RandomHelper.random(), null)) {
          itemStack.shrink(1);
        }
  ...
  public static class Factory
    ...
    public IRecipe parse(JsonContext context, JsonObject json) {
      ...
      return new ScrapingPeltRecipe(group.isEmpty() ? null : new ResourceLocation(group), ingredients, itemstack);
    }
  }
}

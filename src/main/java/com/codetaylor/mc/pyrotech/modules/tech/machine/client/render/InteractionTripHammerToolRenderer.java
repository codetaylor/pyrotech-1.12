...
public class InteractionTripHammerToolRenderer
    implements IInteractionRenderer<TileTripHammer.InteractionTool> {

  public static final InteractionTripHammerToolRenderer INSTANCE = new InteractionTripHammerToolRenderer();

  @Override
  public void renderSolidPass(TileTripHammer.InteractionTool interaction, World world, RenderItem renderItem, BlockPos pos, IBlockState blockState, float partialTicks) {
    ...
    if (!interaction.isEmpty()) {
      ...
      GlStateManager.pushMatrix();
      {
        ...
        TileTripHammer tile = interaction.getTile();
        double remainingToolAnimationTime = tile.getRemainingToolAnimationTime();
        double totalToolAnimationTime = tile.getTotalToolAnimationTime();

        if (remainingToolAnimationTime > 0) {

          if (!Minecraft.getMinecraft().isGamePaused()) {
            tile.setRemainingToolAnimationTime(remainingToolAnimationTime - partialTicks);
          }
          ...
          if (remainingToolAnimationTime > totalToolAnimationTime - downTicks) {
            angle = bounce(totalToolAnimationTime - remainingToolAnimationTime, 0, travelAngle, downTicks);

          } else {
            angle = bounce(totalToolAnimationTime - remainingToolAnimationTime - downTicks, travelAngle, -travelAngle, totalToolAnimationTime - downTicks);
          }
  ...
  @Override
  public void renderSolidPassText(TileTripHammer.InteractionTool interaction, World world, FontRenderer fontRenderer, int yaw, Vec3d offset, BlockPos pos, IBlockState blockState, float partialTicks) {
    //
  }

  @Override
  public boolean renderAdditivePass(TileTripHammer.InteractionTool interaction, World world, RenderItem renderItem, EnumFacing hitSide, Vec3d hitVec, BlockPos hitPos, IBlockState blockState, ItemStack heldItemMainHand, float partialTicks) {
    ...
    if (interaction.isEmpty()
      ...
      if (!itemStack.isEmpty() && remainingToolAnimationTime <= 0) {
  ...
  private double lerp(double t, double b, double c, double d) {

...
public class VanillaFurnaceRecipesRemove {
  ...
  public static void apply() {
    ...
    for (String itemString : ModulePyrotechConfig.RECIPES.VANILLA_FURNACE_REMOVE) {
      ...
      try {
        ...
        for (Iterator<Map.Entry<ItemStack, ItemStack>> it = FurnaceRecipes.instance().getSmeltingList().entrySet().iterator(); it.hasNext(); ) {
          ...
          if (registryName == null) {
            ModulePyrotech.LOGGER.error("Null registry name for item: " + itemStack);
            continue;
          }
        ...
        ModulePyrotech.LOGGER.error("Unable to remove vanilla furnace recipe for output: " + itemString, e);
      }
    }

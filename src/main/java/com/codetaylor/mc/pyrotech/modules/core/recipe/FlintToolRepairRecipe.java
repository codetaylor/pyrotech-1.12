...
public class FlintToolRepairRecipe
    extends ShapelessOreRecipe {

  private final double repairPercentage;

  public FlintToolRepairRecipe(ResourceLocation group, NonNullList<Ingredient> input, @Nonnull ItemStack result, double repairPercentage) {
  ...
  public NonNullList<ItemStack> getRemainingItems(InventoryCrafting inv) {
    ...
    for (int i = 0; i < result.size(); i++) {
      ...
      if (item == ModuleTool.Items.FLINT_TOOL_REPAIR_KIT) {
        hammerRepairDamage = ModuleToolConfig.FLINT_TOOL_REPAIR_KIT.HAMMER_REPAIR_DAMAGE;
      }
    }
    ...
    for (int i = 0; i < result.size(); i++) {
      ...
      } else if (itemStack.getItem() == ModuleTool.Items.FLINT_TOOL_REPAIR_KIT) {
  ...
  public static class Factory
    ...
    public IRecipe parse(JsonContext context, JsonObject json) {
      ...
      return new FlintToolRepairRecipe(group.isEmpty() ? null : new ResourceLocation(group), ingredients, itemstack, ModuleToolConfig.FLINT_TOOL_REPAIR_KIT.PERCENTAGE_DURABILITY_REPAIRED);
    }
  }
}
